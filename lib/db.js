...
export async function fetchGroups(userId) {
  ...
  // 1. Resolve the user's group ids first (avoid passing a query builder to .in())
  ...
  // 2. Fetch groups with nested members + expenses
...
export async function saveUserCategory(userId, cat) {
  const { error } = await getSupabase()
    .from('user_categories')
    .insert({ id: cat.id, user_id: userId, label: cat.label, icon: cat.icon, color: cat.color });
  if (error) throw error;
}
